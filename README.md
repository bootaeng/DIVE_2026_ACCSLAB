...
짐 상황(무게·개수)과 여행 스타일을 입력하면 AI 컨시어지가 **짐캐리 보관·배송 시점까지 포함된 당일치기~다박 일정**을 설계합니다. 물품보관함 요금·혼잡 시간대는 부산교통공사 실데이터를 반영합니다.

| 일정 입력 | AI 생성 일정 |
|:---:|:---:|
| ![여행 일정 입력 화면](docs/images/trip-planner-input.png) | ![AI가 생성한 1일 짐 없는 부산 여정](docs/images/trip-itinerary.png) |

생성된 일정은 **카카오맵 기반 동선 지도**로 시각화되고(장소 자동 지오코딩·구간별 실도로 이동시간), 짐캐리 이용 시 총 짐 처리 비용까지 산정해 줍니다.
...
시간대별 승하차량 예측과 엘리베이터 상태를 융합해 **교통약자와 짐배송 카트의 충돌을 예방하는 안전 경로**를 탐색합니다. LightGBM 혼잡 예측(nocarrier-core) 기반으로 경로 상 모든 역을 스코어링하고, 짐캐리에 짐을 맡겼을 때 이동 부하가 얼마나 낮아지는지 비교해 보여줍니다.

![배리어프리 경로 탐색과 이동 부하·자원 충돌 점수](docs/images/barrier-free-routing.png)

엘리베이터 고장 시 **대체 이동 경로 실데이터**로 우회를 안내하고, 승강장 연단간격이 넓어 휠체어·대형 수하물에 위험한 역은 별도로 경고합니다.
...
혼잡 지표(평균 40% + 최대 병목 60%) → 0~100 압축 → 고장 엘리베이터·위험 연단간격 실시간 가중치 → 짐 소지 가중치 순으로 계산합니다.
...
역 이름을 넣으면 혼잡도·시설·경로 실데이터를 근거로 인용하며 답하는 **RAG 기반 챗봇**입니다. NVIDIA LLM + 임베딩 재랭킹, 네이버 웹검색 근거를 결합했습니다.

<div align="center"><img src="docs/images/ai-assistant-chat.png" width="420" alt="부산 여행 AI 어시스턴트 챗봇" /></div>

### 📊 종합 분석 대시보드

역별 혼잡도 추이와 주요거점·권역별 짐배송 이동 흐름을 Chart.js로 시각화합니다.
...
자세한 설계 배경은 [nocarrier-core README](code/nocarrier-core/README.md)를 참고하세요.
...
**1. 환경변수** — `code/backend/.env.example`을 `code/backend/.env`로 복사하고 API 키를 채워 넣으세요. (휴메트로·네이버·NVIDIA·카카오 — 발급처는 예시 파일 주석 참고)

**2. 데이터** — `참가자_제공_데이터/`와 `code/nocarrier-core/data/`는 저장소에 포함되지 않습니다. 팀 내부 채널로 공유받아 같은 경로에 배치하세요.
